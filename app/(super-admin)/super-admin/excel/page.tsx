  ...
  async function handleDelete(batch: Batch) {
    const warning =
      batch.generatedCount > 0
        ? `Batch "${batch.batchCode}" has ${batch.generatedCount} already-generated certificate(s) -- deleting it will permanently remove those candidates and their QR verification links will stop working. Delete anyway?`
        : `Delete batch "${batch.batchCode}" and all its candidates? This cannot be undone.`;
    if (!confirm(warning)) return;

    const res = await fetch(`/api/organization/excel/${batch._id}`, { method: "DELETE" });
    if (!res.ok) {
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <form onSubmit={handleSubmit} className={cardClass}>
          ...
          <p className="mb-4 text-xs text-zinc-500">
            Required columns: Certificate No, Name, Father Name, DOB, Enrolment No, Course,
            Duration, Grade, Start Date, End Date, Issue Date, Remarks.
          </p>
